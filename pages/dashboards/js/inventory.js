const fetchStock = async () => {
    try {
      const response = await fetch('../../functions/fetchStock.php');
  ...
  const fetchStockSerials = async () => {
    try {
      const response = await fetch('../../functions/fetchStockSerials.php');
      if (!response.ok) throw new Error('ERROR Fetching Stock Serials');
  
      const stockSerials = await response.json();
      return stockSerials;
    } catch (error) {
      console.error(error);
    }
  ...
  const mergeStockData = (stockData, stockSerials) => {
    return stockData.map(stockItem => {
      // Filter serials that match the item_code of the current stock item
      const matchingSerials = stockSerials
        .filter(serial => serial.item_code === stockItem.item_code)
        .map(serial => ({
          id: serial.id,
          eserial: serial.serial, // Renaming serial number field
          status: 'Available', // Default status
          reserved_by: null // Default reserved_by
        }));
  
      return {
        ...stockItem,
        aQty: stockItem.qty - stockItem.rQty, // Calculate available quantity
  ...
  fetchStock().then(stockData => {
    fetchStockSerials().then(stockSerials => {
      const combinedData = mergeStockData(stockData, stockSerials);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const serialNumbersModal = new bootstrap.Modal(
    document.getElementById('serialNumbersModal')
  )
      ...
      items: {
        ...
        edit_stock: {
          ...
          callback: function (key, selection) {
            ...
            const newRQty = prompt(
              'Enter new reserved quantity:',
              selectedStock.rQty
            )
            const newRRep = prompt(
              'Enter new reserved representative:',
              selectedStock.rRep
            )
