...
function fetchMasterInvoice() {
    fetch('../../functions/fetchMasterInv.php')
        ...
        .then(data => {
            ...
            const response = data.filter(invoice => !invoice.gin && !invoice.grn);

            // Display the filtered data
            //console.log('Filtered Data:', response);            
            response.forEach(x => {
                fetchedDate.push(response);
                invoiceData.push({
                    date: x.date,
                    invoice: x.inv,
                    customer: x.customer,
                    ...
                    other: parseFloat(x.object || 0),
                    total: parseFloat(x.value) + parseFloat(x.vat) + parseFloat(x.object || 0)
                });
            });

            displayInvoices(invoiceData);
            populateFilters();
            populateCusData(response);
            populateCusGpData(response);
            populateRepSalesData(response);
            populateRepGpData(response);
            populatepProSaleData(response);
            populatepProGpData(response);
...
function displayChart(response) {
    ...
    function topProducts(reportData) {
        ...
        const totalValue = Object.values(totalValuesByProduct).reduce((acc, curr) => acc + curr, 0);
        ...
        if (sortedProducts.length > 0) {
            ...
            document.getElementById('top1CategoryValue').textContent = `${top1Percentage}%`;
            document.getElementById('top1CategoryProgress').style.width = `${top1Percentage}%`;
        }
        ...
        if (sortedProducts.length > 1) {
            ...
            document.getElementById('top2CategoryValue').textContent = `${top2Percentage}%`;
            document.getElementById('top2CategoryProgress').style.width = `${top2Percentage}%`;
        }
...
function sortTable(columnIndex) {
    ...
    rows.sort((a, b) => {
        ...
        if (aValue.includes('$')) {
            return sortDirection * (parseFloat(aValue.replace('$', '')) - parseFloat(bValue.replace('$', '')));
        }
...
function groupDataByCustomerAndRep(data) {
    ...
    Object.values(groupedData).forEach(row => {
        row.total = ['jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec']
...
function displayData(groupedData) {
    ...
    hot = new Handsontable(document.getElementById("salesTable"), {
        ...
        columns: [
            ...
            { data: 'apr', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'may', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'jun', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'jul', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'aug', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'sep', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'oct', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'nov', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'dec', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'total', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' }, renderer: totalRenderer }
        ],
        colHeaders: [
            'Customer Name', 'Rep', 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun',
...
function groupDataByCustomerAndRepGp(data) {
    ...
    data.forEach(item => {
        ...
        if (!groupedData[key]) {
            groupedData[key] = {
                customer: item.customer,
                rep: item.rep || 'N/A',
                gp_jan: 0, gp_feb: 0, gp_mar: 0, gp_apr: 0, gp_may: 0, gp_jun: 0,
                gp_jul: 0, gp_aug: 0, gp_sep: 0, gp_oct: 0, gp_nov: 0, gp_dec: 0,
                total_gp: 0 // Initialize GP total to 0
            };
        }

        const month = new Date(item.date).getMonth();
        const gpMonthKeys = ['gp_jan', 'gp_feb', 'gp_mar', 'gp_apr', 'gp_may', 'gp_jun', 'gp_jul', 'gp_aug', 'gp_sep', 'gp_oct', 'gp_nov', 'gp_dec'];
    ...
    Object.values(groupedData).forEach(row => {
        row.total_gp = ['gp_jan', 'gp_feb', 'gp_mar', 'gp_apr', 'gp_may', 'gp_jun', 'gp_jul', 'gp_aug', 'gp_sep', 'gp_oct', 'gp_nov', 'gp_dec']
            .reduce((sum, month) => {
...
function displayGpData(groupedData) {
    const tableData = Object.values(groupedData).filter(row => {
        return (
            row.gp_jan > 0 || row.gp_feb > 0 || row.gp_mar > 0 || row.gp_apr > 0 ||
            row.gp_may > 0 || row.gp_jun > 0 || row.gp_jul > 0 || row.gp_aug > 0 ||
            row.gp_sep > 0 || row.gp_oct > 0 || row.gp_nov > 0 || row.gp_dec > 0
        );
    ...
    cusGpHot = new Handsontable(document.getElementById("gpTable"), {
        ...
        columns: [
            ...
            { data: 'gp_jan', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'gp_feb', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'gp_mar', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'gp_apr', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'gp_may', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'gp_jun', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'gp_jul', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'gp_aug', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'gp_sep', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'gp_oct', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'gp_nov', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'gp_dec', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
            { data: 'total_gp', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' }, renderer: gpTotalRenderer }
        ],
        colHeaders: [
            'Customer Name', 'Rep', 'Jan GP', 'Feb GP', 'Mar GP', 'Apr GP', 'May GP', 'Jun GP',
            'Jul GP', 'Aug GP', 'Sep GP', 'Oct GP', 'Nov GP', 'Dec GP', 'Total GP'
        ],
        width: '100%',
        height: 450,
        rowHeaders: false,
...
function populateRepSalesData(data) {
    document.getElementById('rep-sale-tab').addEventListener('click', function () {
...
function repSaleChart(groupedData) {
    ...
    document.getElementById('repSaleTotal').textContent = `Total Sale: RS. ${totalSales.toLocaleString()}`;
...
function groupDataByRepSales(data) {
    ...
    data.forEach(item => {
        ...
        if (!groupedData[rep]) {
            groupedData[rep] = {
                rep: rep,
                sales_jan: 0, sales_feb: 0, sales_mar: 0, sales_apr: 0,
                sales_may: 0, sales_jun: 0, sales_jul: 0, sales_aug: 0,
                sales_sep: 0, sales_oct: 0, sales_nov: 0, sales_dec: 0,
                total_sales: 0
            };
        }
        const month = new Date(item.date).getMonth();
        const salesMonthKeys = ['sales_jan', 'sales_feb', 'sales_mar', 'sales_apr', 'sales_may', 'sales_jun', 'sales_jul', 'sales_aug', 'sales_sep', 'sales_oct', 'sales_nov', 'sales_dec'];
    ...
    Object.values(groupedData).forEach(row => {
        row.total_sales = ['sales_jan', 'sales_feb', 'sales_mar', 'sales_apr', 'sales_may', 'sales_jun', 'sales_jul', 'sales_aug', 'sales_sep', 'sales_oct', 'sales_nov', 'sales_dec']
    ...
    if (repSalesHot) {
        ...
        repSalesHot = new Handsontable(document.getElementById("repSalesTable"), {
            ...
            columns: [
                ...
                { data: 'sales_jan', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: 'sales_feb', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: 'sales_mar', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: 'sales_apr', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: 'sales_may', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: 'sales_jun', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: 'sales_jul', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: 'sales_aug', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: 'sales_sep', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: 'sales_oct', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: 'sales_nov', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: 'sales_dec', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: 'total_sales', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } }
            ],
            colHeaders: [
                'Rep', 'Jan Sales', 'Feb Sales', 'Mar Sales', 'Apr Sales', 'May Sales', 'Jun Sales',
                'Jul Sales', 'Aug Sales', 'Sep Sales', 'Oct Sales', 'Nov Sales', 'Dec Sales', 'Total Sales'
            ],
            width: '100%',
            height: 'auto',
            rowHeaders: false,
            ...
            cells: function(row, col, prop) {
                ...
                if (row === tableData.length - 1) {  // Changed from data.length to tableData.length
...
function groupRepGpData(data) {
    ...
    data.forEach(item => {
        ...
        if (!groupedData[rep]) {
            groupedData[rep] = {
                rep: rep,
                total_gp_jan: 0, total_gp_feb: 0, total_gp_mar: 0, total_gp_apr: 0,
                total_gp_may: 0, total_gp_jun: 0, total_gp_jul: 0, total_gp_aug: 0,
                total_gp_sep: 0, total_gp_oct: 0, total_gp_nov: 0, total_gp_dec: 0,
                total_gp: 0
            };
        }
        const month = new Date(item.date).getMonth();
        const gpMonthKeys = ['total_gp_jan', 'total_gp_feb', 'total_gp_mar', 'total_gp_apr', 'total_gp_may', 'total_gp_jun', 'total_gp_jul', 'total_gp_aug', 'total_gp_sep', 'total_gp_oct', 'total_gp_nov', 'total_gp_dec'];
    ...
    Object.values(groupedData).forEach(row => {
        row.total_gp = ['total_gp_jan', 'total_gp_feb', 'total_gp_mar', 'total_gp_apr', 'total_gp_may', 'total_gp_jun', 'total_gp_jul', 'total_gp_aug', 'total_gp_sep', 'total_gp_oct', 'total_gp_nov', 'total_gp_dec']
            .reduce((sum, month) => sum + (row[month] || 0), 0);
    });
...
function displayRepGpData(data) {
    ...
    if (!repGpHot) {
        ...
        repGpHot = new Handsontable(container, {
            data: data,
            colHeaders: ["Rep", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec", "Total GP"],
            columns: [
                { data: "rep", readOnly: true },
                { data: "total_gp_jan", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_gp_feb", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_gp_mar", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_gp_apr", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_gp_may", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_gp_jun", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_gp_jul", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_gp_aug", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_gp_sep", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_gp_oct", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_gp_nov", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_gp_dec", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_gp", type: 'numeric', readOnly: true, className: "htBold htRight", numericFormat: { pattern: '0,0.00' } }
            ],
            width: '100%',
            height: 450,
            rowHeaders: false,
...
function populatepProSaleData(data) { 
    document.getElementById('pro-sale-tab').addEventListener('click', function () {
...
function groupProSaleData(data) {
    ...
    data.forEach(item => {
        ...
        if (!groupedData[brand]) {
            groupedData[brand] = {
                ...
                total_product_sale_jan: 0, total_product_sale_feb: 0, total_product_sale_mar: 0, total_product_sale_apr: 0,
                total_product_sale_may: 0, total_product_sale_jun: 0, total_product_sale_jul: 0, total_product_sale_aug: 0,
                total_product_sale_sep: 0, total_product_sale_oct: 0, total_product_sale_nov: 0, total_product_sale_dec: 0,
                total_product_sale: 0
            };
        }
        const month = new Date(item.date).getMonth();
        const proSaleMonthKeys = ['total_product_sale_jan', 'total_product_sale_feb', 'total_product_sale_mar', 'total_product_sale_apr', 'total_product_sale_may', 'total_product_sale_jun', 'total_product_sale_jul', 'total_product_sale_aug', 'total_product_sale_sep', 'total_product_sale_oct', 'total_product_sale_nov', 'total_product_sale_dec'];
    ...
    Object.values(groupedData).forEach(row => {
        row.total_product_sale = ['total_product_sale_jan', 'total_product_sale_feb', 'total_product_sale_mar', 'total_product_sale_apr', 'total_product_sale_may', 'total_product_sale_jun', 'total_product_sale_jul', 'total_product_sale_aug', 'total_product_sale_sep', 'total_product_sale_oct', 'total_product_sale_nov', 'total_product_sale_dec']
            .reduce((sum, month) => sum + (row[month] || 0), 0);
    });

    // Convert groupedData object to an array and filter out rows with zero totals
    const dataArray = Object.values(groupedData).filter(row => row.total_product_sale > 0);
...
function displayProSaleData(data) {
    ...
    if (!proSaleHot) {
        ...
        proSaleHot = new Handsontable(container, {
            data: data,
            colHeaders: ["Product", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec", "Total Sale"],
            columns: [
                { data: "product", readOnly: true },
                { data: "total_product_sale_jan", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_sale_feb", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_sale_mar", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_sale_apr", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_sale_may", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_sale_jun", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_sale_jul", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_sale_aug", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_sale_sep", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_sale_oct", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_sale_nov", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_sale_dec", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_sale", type: 'numeric', readOnly: true, className: "htBold htRight", numericFormat: { pattern: '0,0.00' } }
            ],
            width: '100%',
            height: 450,
            rowHeaders: false,
...
function groupProGpData(data) {
    ...
    data.forEach(item => {
        ...
        if (!groupedData[brand]) {
            groupedData[brand] = {
                product: brand,
                total_product_gp_jan: 0, total_product_gp_feb: 0, total_product_gp_mar: 0, total_product_gp_apr: 0,
                total_product_gp_may: 0, total_product_gp_jun: 0, total_product_gp_jul: 0, total_product_gp_aug: 0,
                total_product_gp_sep: 0, total_product_gp_oct: 0, total_product_gp_nov: 0, total_product_gp_dec: 0,
                total_product_gp: 0
            };
        }
        const month = new Date(item.date).getMonth();
        const proSaleMonthKeys = ['total_product_gp_jan', 'total_product_gp_feb', 'total_product_gp_mar', 'total_product_gp_apr', 'total_product_gp_may', 'total_product_gp_jun', 'total_product_gp_jul', 'total_product_gp_aug', 'total_product_gp_sep', 'total_product_gp_oct', 'total_product_gp_nov', 'total_product_gp_dec'];
    ...
    Object.values(groupedData).forEach(row => {
        row.total_product_gp = ['total_product_gp_jan', 'total_product_gp_feb', 'total_product_gp_mar', 'total_product_gp_apr', 'total_product_gp_may', 'total_product_gp_jun', 'total_product_gp_jul', 'total_product_gp_aug', 'total_product_gp_sep', 'total_product_gp_oct', 'total_product_gp_nov', 'total_product_gp_dec']
            .reduce((sum, month) => sum + (row[month] || 0), 0);
    });

    const dataArray = Object.values(groupedData).filter(row => row.total_product_gp > 0);
...
function displayProGpData(data) {
    ...
    if (!proGpHot) {
        ...
        proGpHot = new Handsontable(container, {
            data: data,
            colHeaders: ["Product", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec", "Total GP"],
            columns: [
                { data: "product", readOnly: true },
                { data: "total_product_gp_jan", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_gp_feb", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_gp_mar", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_gp_apr", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_gp_may", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_gp_jun", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_gp_jul", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_gp_aug", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_gp_sep", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_gp_oct", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_gp_nov", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_gp_dec", type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
                { data: "total_product_gp", type: 'numeric', readOnly: true, className: "htBold htRight", numericFormat: { pattern: '0,0.00' } }
            ],
            width: '100%',
            height: 450,
            rowHeaders: false,
...
function populateOverView(fetchedData) {
    document.getElementById('overView-tab').addEventListener('click', function () {
        ...
        const columns = [
            ...
            { title: 'Total', type: 'numeric', numericFormat: { pattern: '0,0.00' }, readOnly: true }
        ];
        ...
        overViewHot = new Handsontable(container, {
            ...
            cells(row, col) {
                ...
                if (row === 2) {
                    cellProperties.readOnly = true;
                    cellProperties.numericFormat = { pattern: '0.000%', culture: 'en-US' };
                }
...
function getTableData(fetchedData) {
  const data = [
    ['Sales', null, null, null, null, null, null, null, null, null, null, null, null, null],
    ['GP', null, null, null, null, null, null, null, null, null, null, null, null, null],
  ...
  for (let row = 0; row < 2; row++) {
    data[row][13] = data[row].slice(1, 13).reduce((sum, val) => sum + (val || 0), 0);
  }
