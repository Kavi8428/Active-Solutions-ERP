...
function populateHandsonTable () {
  ...
  hot = new Handsontable(container, {
    // Initialize Handsontable globally
    data: getInitialData(),
...
function populateGrnItems (selectedId) {
  ...
  fetch('../../functions/fetchGrnItem.php')
    ...
    .then(response => {
      ...
      // console.log('selectedId', selectedId.selectedGrnId);

      const matchedData = response.filter(
        item => item.grn_id === selectedId.selectedGrnId
      )
...
function sendData () {
  ...
  grnItems.forEach(function (item) {
    const serialCount = item.SERIALS
      ? item.SERIALS.split(',').filter(tag => tag.trim()).length
      : 0
  ...
  fetch('../../functions/fetchGrnData.php')
    ...
    .then(response => {
      ...
      if (urlValue && urlValue.urlId) {
        ...
        const matchedData = response.find(
