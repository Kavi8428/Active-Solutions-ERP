                ...
                <div class="col-12">
                                    ...
                                    <div class="row" style="margin:10px; font-size: small; ">
                                        <div id="letterhead" class="row">
                                            ...
                                            <span style="text-align: right; font-size: small; color:black; " class="col-6 ">
                                                ...
                                                <span id="ourVat">VAT No : <?php echo $activeVat ?></span>
                                            </span>
                                        ...
                                        <div class="row">
                                            <div class="col-10">
                                                <input style=" font-size: 13px; border: none;" class=" w-100" type="text"
                                                    value=" <?php echo $partner_name ?>" readonly>
                                            </div>
                                                    ...
                                                    <table id="mainTable" class=" col-12">
                                                        <thead>
                                                            <tr style="background-color: rgb(33,37,41); color: white; " class="row p-1 ">
                                                                ...
                                                                <th id="tableVatArea" class="col-1 text-center">VAT</th>
                                                            </tr>
                                                        </thead>
                                                        ...
                                                        <tfoot style="color: black;">
                                                            <tr class="row" id="subtotalArea">
                                                                <td id="subTotalComment" class="col-6 pt-2"></td>
                                                                <td class="col-2 pt-2 text-start border-start border-end border-bottom table-footer text-bold ">SUBTOTAL</td>
                                                                ...
                                                                <td id="subTotalLastRow" class="col-1 text-start ps-2 pt-2"></td>
                                                            </tr>
                                                            <tr class="row" id="discountArea">
                                                                ...
                                                                <td class="col-1 pt-2 text-start border-end border-bottom"></td>
                                                                <td id="discount" class="col-2 pt-2 text-end border-end border-bottom"></td>
                                                                <td id="discountLastRow" class="col-1 text-start ps-2 pt-2"></td>
                                                            </tr>
                                                            <tr class="row" id="vatArea">
                                                                ...
                                                                <td id="vatLastRow" class="col-1 text-start ps-2 pt-2"></td>
                                                            </tr>
                                                            <tr class="row" id="totalArea">
                                                                <td id="totalComment" class="col-6 pt-2"></td>
                                                                <td class="col-2 pt-2 text-start border-start border-end border-bottom table-footer text-bold">TOTAL</td>
                                                                <td class="col-1 pt-2 text-start border-end border-bottom"></td>
                                                                <td id="grandTotal" class="col-2 pt-2 text-end border-end border-bottom table-footer"></td>
                                                                <td id="grandTotalLastRow" class="col-1 text-start ps-2 pt-2"></td>
                                                            </tr>
                                                        </tfoot>
                                                    </table>
                                                </div>
                    ...
                    <script>
                       ...
                       document.getElementById('edit').addEventListener('click', function() {
                            location.href = '../../pages/dashboards/qt_edit.php?code=' + dealId;
                        });
                        ...
                        (async () => {
                            try {
                                ...
                                console.log('filteredDataCheck0',filteredDataCheck0)
                        ...
                        function populateTable(filteredData, filteredDataCheck0) {
                            ...
                            const filteredQuotation1Data = fetchQuotes.filter(item => item.deal_id === dealId);
                            const discount1Value = filteredQuotation1Data.reduce((sum, item) => sum + parseFloat(item.discount), 0);
                            let comment;

                            fetchQuotes.map(item => {
                                comment = item.comment; // Use loose equality (==) to handle type differences
                            });

                            // Flag to indicate the first row for each quotation
                            let isFirstRow = true;

                            const hasDiscount = fetchQuotes.some(item => {
                                return item.discounted == 0; // Use loose equality (==) to handle type differences
                            });

                            const hasVat = fetchQuotes.some(item => {
                                return item.vated == 0; // Use loose equality (==) to handle type differences
                            });

                            itemsCount = filteredData.length;
                            // Execute the loop to process each item
                            filteredData.forEach(dataItem => {

                                let amount = 0;
                                let itemVAT = 0;
                                let rate = isFirstRow ? parseFloat(dataItem.price) + chk0rate : parseFloat(dataItem.price);

                                // Adjust UI visibility based on discount and VAT
                                if (hasDiscount && hasVat) {

                                    console.log('both');

                                    document.getElementById('subTotalComment').textContent = comment;
                                    if (!footerAdded) {
                                        addFooterCells();
                                        footerAdded = true; // Set the flag to true after adding footer cells
                                        // console.log('Footer cells added.');
                                    }
                                    //console.log('Show discount & vat');
                                    updateFooterColumnSizes('col-7', 'col-6');
                                    // Calculate the amount and VAT for each item
                                    vatedRate = rate * (parseFloat(dataItem.vat) / 100);
                                    totalVat = vatedRate * dataItem.quantity;
                                    vat += totalVat;
                                    vatElement.textContent = formatCurrency(vat);
                                    amount = (rate) * dataItem.quantity;
                                    // Accumulate the subtotal and VAT
                                    subTotal += amount;
                                    // totalVAT += itemVAT;
                                    discount.textContent = formatCurrency(discount1Value);
                                    // Calculate grand total
                                    const grandTotal = (subTotal - discount1Value) + vat;
                                    ...
                                    newRow.innerHTML = `
                                        ...
                                        <td class="col-4 pt-1 text-start border-start border-end border-bottom">${formattedDescription}</td>
                                        ...
                                        <td class="col-1 pt-1 text-end border-start border-end border-bottom">${dataItem.vat}%</td>
                                    `;
                                    ...
                                    console.log('!hasDiscount && hasVat');

                                    rate = isFirstRow ? (parseFloat(dataItem.price) + chk0rate) - discount1Value : parseFloat(dataItem.price);
                                    ...
                                    document.getElementById('cusVat').hidden = true;
                                    document.getElementById('ourVat').hidden = true;
                                    document.getElementById('discountArea').hidden = true;
                                    document.getElementById('vatArea').hidden = true;
                                    document.getElementById('tableVatArea').hidden = true;
                                    document.getElementById('subtotalArea').hidden = true;

                                    const vatCells = document.querySelectorAll('.tableVatAreaValue');
                                    vatCells.forEach(cell => {
                                        cell.style.visibility = 'hidden';
                                    });

                                    vatedRate = rate * (dataItem.vat / 100);
                                    rate += vatedRate;
                                    amount = (rate) * dataItem.quantity;
                                    // Accumulate the subtotal and VAT
                                    subTotal += amount;
                                    ...
                                    updateFooterColumnSizes('col-6', 'col-7');

                                    const descriptionHeader = document.querySelector("#mainTable th:nth-child(2)");
                                    if (descriptionHeader) {
                                        descriptionHeader.classList.remove("col-4");
                                        descriptionHeader.classList.add("col-5");
                                    }
                                    ...
                                    newRow.innerHTML = `
                                        <td class="col-2 pt-1 text-start border-start border-end border-bottom">${dataItem.item_code}</td>
                                        <td class="col-5 pt-1 text-start border-start border-end border-bottom">${formattedDescription}</td>
                                        <td class="col-2 pt-1 text-end border-start border-end border-bottom">${formatCurrency(rate)}</td>
                                        <td class="col-1 pt-1 text-center border-start border-end border-bottom">${dataItem.quantity}</td>
                                        <td class="col-2 pt-1 text-end border-start border-end border-bottom">${formatCurrency(amount)}</td>
                                    `;
                                    ...
                                    discount.textContent = formatCurrency(discount1Value);
                                    // Calculate grand total
                                    const grandTotal = subTotal - discount1Value;
                                    // Display the calculated values in the respective table footer cells
                                    subTotalElement.textContent = formatCurrency(subTotal);
                                    //vatElement.textContent = formatCurrency(totalVAT);
                                    grandTotalElement.textContent = formatCurrency(grandTotal);
                        ...
                        var vatInput = document.getElementById('vat').textContent;
                        var grandTotalInput = document.getElementById('grandTotal').textContent;

                        // Add input event listener to update VAT value in real-time

                        function sendValuesToPHP() {
                            // Get the VAT and grand total values
                            var vatValue = extractNumericValue(vatInput);
                            var grandTotalValue = extractNumericValue(grandTotalInput);
                        ...
                        // Store original HTML content of td elements
                        const originalSubTotalHTML = '<td id="subTotalLastRow" class="col-1 text-start ps-2 pt-2"></td>';
                        const originalVatHTML = '<td id="vatLastRow" class="col-1 text-start ps-2 pt-2">VAT</td>';
                        const originalDiscountHTML = '<td id="discountLastRow" class="col-1 text-start ps-2 pt-2"></td>';
                        const originalGrandTotalHTML = '<td id="grandTotalLastRow" class="col-1 text-start ps-2 pt-2"></td>';

                        // Function to remove specific td elements from footer
                        function removeFooterCells() {
                            document.getElementById('subTotalLastRow').remove();
                            document.getElementById('vatLastRow').remove();
                            document.getElementById('discountLastRow').remove();
                            document.getElementById('grandTotalLastRow').remove();
                        }

                        // Function to add td elements back to the footer
                        function addFooterCells() {
                            const subtotalRow = document.getElementById('subtotalArea');
                            const discountRow = document.getElementById('discountArea');
                            const vatRow = document.getElementById('vatArea');
                            const totalRow = document.getElementById('totalArea');

                            subtotalRow.insertAdjacentHTML('beforeend', originalSubTotalHTML);
                            discountRow.insertAdjacentHTML('beforeend', originalDiscountHTML);
                            vatRow.insertAdjacentHTML('beforeend', originalVatHTML);
                            totalRow.insertAdjacentHTML('beforeend', originalGrandTotalHTML);
                        }


                        function updateFooterColumnSizes(oldClass, newClass) {
                            // Select all rows in the <tfoot> using the row IDs defined
                            const subtotalRow = document.querySelector("#subtotalArea td:nth-child(1)");
                            const discountRow = document.querySelector("#discountArea td:nth-child(1)");
                            const vatRow = document.querySelector("#vatArea td:nth-child(1)");
                            const totalRow = document.querySelector("#totalArea td:nth-child(1)");

                            // Change column size for 'SUBTOTAL' row
                            if (subtotalRow) {
                                subtotalRow.classList.remove(oldClass);
                                subtotalRow.classList.add(newClass);
                            }

                            // Change column size for 'DISCOUNT' row
                            if (discountRow) {
                                discountRow.classList.remove(oldClass);
                                discountRow.classList.add(newClass);
                            }

                            // Change column size for 'VAT' row
                            if (vatRow) {
                                vatRow.classList.remove(oldClass);
                                vatRow.classList.add(newClass);
                            }

                            // Change column size for 'TOTAL' row
                            if (totalRow) {
                                totalRow.classList.remove(oldClass);
                                totalRow.classList.add(newClass);
                            }
                        }
