                        ...
                        <ul class="nav ">
                            ...
                            <li class="nav-item ">
                                ...
                                <div class="collapse " id="productsExample">
                                    <ul class="nav nav-sm flex-column">
                                        <li class="nav-item">
                                            <a class="nav-link text-white " href="../ecommerce/products/products-list.php">
                                                <span class="sidenav-mini-icon"> P </span>
                                                <span class="sidenav-normal  ms-2  ps-1"> Products List </span>
                                            </a>
                                        </li>
                                        <li class="nav-item">
                                            <a class="nav-link text-white " href="../ecommerce/products/new-product.php">
                                                <span class="sidenav-mini-icon"> N </span>
                            ...
                            <li class="nav-item ">
                                <a class="nav-link text-white " data-bs-toggle="collapse" aria-expanded="false" href="#componentsExamples">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        ...
        <nav class="navbar navbar-main navbar-expand-lg position-sticky mt-4 top-1 px-0 mx-4 shadow-none border-radius-xl z-index-sticky" id="navbarBlur" data-scroll="true">
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    ...
                    <h6 class="font-weight-bolder mb-0">Price Details</h6>
                </nav>
                ...
                <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4" id="navbar">
                    ...
                    <ul class="navbar-nav  justify-content-end">
                        <li class="nav-item">
                            <a href="../pages/profile/overview.php" class="nav-link text-body p-0 position-relative" target="_blank">
        ...
        <div class="container-fluid py-4">
            <div style="height: 40px;" class="row">
                ...
                <div class="col-2">
                    <button type="button" class="btn btn-outline-primary btn-sm mb-0" data-toggle="modal" data-target="#importModal">
                        Import
                    </button>
                    <!-- Modal -->
                    <div class="modal fade" id="importModal" tabindex="-1" aria-labelledby="modalTitle" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="modalTitle">Import Excel</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                </div>
                                <div class="modal-body">
                                    <p>You can browse your computer for an Excel file.</p>
                                    <div class="input-group input-group-dynamic mb-3">
                                        <div class="custom-file">
                                            <input type="file" class="custom-file-input" id="excelFileInput" accept=".xlsx, .xls">
                                ...
                                <div class="modal-footer">
                                    ...
                                    <button type="button" class="btn btn-primary" onclick="uploadExcelFile()">Upload</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
function uploadExcelFile() {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            // Check if the file headers match the expected order
            var expectedHeaders = ['ItemCode', 'Description', 'Retail', 'Reseller', 'Low Reseller', 'Cost'];
            console.log('expectedHeaders',expectedHeaders)
            var actualHeaders = [];

            // Loop through the cells in the first row to extract headers
            var range = XLSX.utils.decode_range(worksheet['!ref']);
            for (var C = range.s.c; C <= range.e.c; ++C) {
                var cellAddress = { c: C, r: range.s.r };
                var cellRef = XLSX.utils.encode_cell(cellAddress);
                var cellValue = worksheet[cellRef].v;
                actualHeaders.push(cellValue.trim());
            }

            // Remove any empty headers and convert to lowercase for comparison
            actualHeaders = actualHeaders.filter(header => header !== '').map(header => header.toLowerCase());
            console.log('actualHeaders',actualHeaders)

            // Check if the headers match expected headers
            var headersMatch = expectedHeaders.every((header, index) => header.toLowerCase() === actualHeaders[index]);

            console.log('headersMatch',headersMatch)

            if (!headersMatch) {
                alert('Please ensure the file columns are ordered correctly: ItemCode, Description, Retail, Reseller, Low Reseller, Cost');
                return;
            }

            // Convert the worksheet to a JSON object, skipping empty rows
            ...
            // Remove empty arrays from the JSON data
            jsonData = jsonData.filter(row => row.length > 0);

            // Display selected data (ItemCode, Retail, Reseller, Low Reseller, and Cost)
            displaySelectedData(jsonData);
        };
        reader.readAsArrayBuffer(file);
    } else {
        console.log("No file selected");
        alert('Mmmm... You have not selected a file.');
    }
}
    function displaySelectedData(jsonData) {
        // Assuming jsonData is an array of arrays with each inner array representing a row
        var selectedData = [];
        var itemCodeAndDescription = [];
        for (var i = 0; i < jsonData.length; i++) {
            ...
            // Push item_code and description into itemCodeAndDescription array
            var itemCode = row[0];
            var description = row[1];
            itemCodeAndDescription.push({
                ItemCode: itemCode,
        ...
        // Display item_code and description
        displayItemCodeAndDescription(itemCodeAndDescription);

         $.ajax({
                            url: '../../functions/uploadPrice.php',
                            type: 'POST',
                            data: {
                                jsonData: selectedData
                            }, // Pass the selectedData array to PHP
                            success: function(response) {
                                console.log(response); // Log the response from the server
                                alert('Yeh... Prices are updated')

                                location.reload();
                            },
                            error: function(xhr, status, error) {
                                console.error(error); // Log any errors that occur during the AJAX request
                                alert('Something not going well. Please contact a person who developed this.')
                            }
                        });
                        // Log or display the selected data
                        console.log(selectedData);
                        // You can display the selected data in your HTML as well, e.g., update a table
...
// Function to log item_code and description to console
function displayItemCodeAndDescription(data) {
    console.log("Item Codes and Descriptions:");
    data.forEach(function(item) {
        console.log(`Item Code: ${item.ItemCode}, Description: ${item.Description}`);
    });
    $.ajax({
        url: '../../functions/updateProduct.php', // Update the URL to point to your PHP script
        type: 'POST',
        data: {
            itemCodeAndDescription: data
        },
        success: function(response) {
            console.log(response); // Log the response from the server
            alert('Yeh... Item codes and descriptions are inserted into the database');
            location.reload();
        },
        error: function(xhr, status, error) {
            console.error(error); // Log any errors that occur during the AJAX request
            alert('Something not going well. Please contact a person who developed this.');
                ...
                <div class="col-2">
                    <a href="../../pages/dashboards/price_new.php">
                        <button class="btn btn-dark">New Price</button>
                    </a>
                </div>
            </div>
            <div class="row mt-2">
                        ...
                        <div class="table-responsive">
                            <table class="table table-flush" id="datatable-search">
                                <thead class="thead-light">
                                    <tr>
                                        <th class="text-uppercase text-xs font-weight-bolder opacity-7">Item Code</th>
                                        <th class="text-uppercase text-xxs font-weight-bolder opacity-7 ps-2">Cost</th>
                                        <th class="text-uppercase text-xxs font-weight-bolder opacity-7 ps-2">Low Re-Seller</th>
                                        <th class="text-uppercase text-xxs font-weight-bolder opacity-7 ps-2">Re-Seller</th>
                                        <th class="text-uppercase text-xxs font-weight-bolder opacity-7 ps-2">Retailer</th>
                                ...
                                <tbody>
                                    ...
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        echo '<tr>';
                                        echo '<td style="font-size:12px;">' . $row['item_code'] . '</td>';
                                        echo '<td style="font-size:12px;">' . $row['cost'] . '</td>';
                                        echo '<td style="font-size:12px;">' . $row['lr'] . '</td>';
                                        echo '<td style="font-size:12px;">' . $row['r'] . '</td>';
                                        echo '<td style="font-size:12px;">' . $row['rt'] . '</td>';
                                        echo '<td>';
                                        echo '<a href="price_edit.php?code=' . urlencode($row['item_code']) . '"><button class="btn">Edit</button></a>';
                                        echo '<a href="?delete=true&item_code=' . urlencode($row['item_code']) . '" onclick="return confirm(\'Are you sure you want to delete this data?\');"><button class="btn">Del</button></a>';
                                        echo '</td>';
                                        echo '</tr>';
                                    }
                                    ...
                                    if (isset($_GET['delete']) && $_GET['delete'] == 'true' && isset($_GET['item_code'])) {
                                        // Get the item_code from the URL
                                        $itemCodeToDelete = $_GET['item_code'];

                                        // Fetch the row before deletion
                                        $selectDeletedRowSql = "SELECT * FROM pricing WHERE item_code = ?";
                                        $stmtSelect = $conn->prepare($selectDeletedRowSql);
                                        $stmtSelect->bind_param('s', $itemCodeToDelete);
                                        $stmtSelect->execute();
                                        $deletedRowResult = $stmtSelect->get_result()->fetch_assoc();
                                        $stmtSelect->close();

                                        // Prepare the SQL statement for deletion
                                        $deleteSql = "DELETE FROM pricing WHERE item_code = ?";
                                        $stmt = $conn->prepare($deleteSql);
                                        $stmt->bind_param('s', $itemCodeToDelete);

                                        // Execute the statement
                                        if ($stmt->execute()) {
                                            // Backup part
                                            $backupFileName = 'price_backup_' . date('Y-m-d_H-i-s') . '.doc';
                                            $backupFilePath = '../../assets/price_backup/' . $backupFileName;
                                            $backupContent = "Backup of Pricing Data\n";

                                            // Append data to backup content
                                            if ($deletedRowResult) {
                                                foreach ($deletedRowResult as $key => $value) {
                                                    $backupContent .= ucfirst(str_replace('_', ' ', $key)) . ": " . $value . "\n";
                                                }
                                            }
                                            ...
                                            // Log the deletion
                                            logEvent($_SESSION['user'], 'Delete Price', 'Item Code: ' . $itemCodeToDelete, 1);

                                            // Redirect to the price view page
                                            echo '<script>window.location.href="../../pages/dashboards/price_view.php"</script>';
                                            exit();
                                        } else {
                                            // Failed deletion
                    ...
                    <div class="row align-items-center justify-content-lg-between">
                        ...
                        <div class="col-lg-6">
                            <ul class="nav nav-footer justify-content-center justify-content-lg-end">
                                <li class="nav-item">
                                    <a href="https://www.activelk.com" class="nav-link text-muted" target="_blank">activelk</a>
                                </li>
                                <li class="nav-item">
        ...
        <div class="card shadow-lg">
            ...
            <div class="card-body pt-sm-3 pt-0">
                ...
                <div class="w-100 text-center">
                    <a class="github-button" href="../../dashboard.php" data-icon="octicon-star" data-size="large" data-show-count="true" aria-label="Star creativetimofficial/material-dashboard on GitHub">Star</a>
