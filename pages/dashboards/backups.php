    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
            ...
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm text-dark active" aria-current="page"> <a href="../../log.php">Log</a> </li>
                    </ol>
                    <h6 class="font-weight-bolder mb-0">Backup Details</h6>
                </nav>
        ...
        <div class="container-fluid py-4">
            <div style="height: 40px;" class="row">
                <div class="col-10">
                    <h5 class="mb-0">Backuped Files</h5>
                </div>
                <div class="col-2">
                <button class="btn btn-dark" onclick="clearSpace()">Clear Space</button>
                </div>
            </div>
            <div class="row mt-2">
                <div class="col-12">
...
$backupPaths = [
    '../../assets/backup/',
    '../../assets/price_backup/',
    '../../assets/product_backup/',
    '../../assets/customer_backup/',
    '../../assets/customer_employee/'
];

$allFiles = [];

foreach ($backupPaths as $backupPath) {
    $files = glob($backupPath . '*.doc');

    foreach ($files as $file) {
        $allFiles[] = [
            'folder' => basename($backupPath),
            'file' => basename($file)
        ];
    }
}
...
<table class="table table-flush" id="datatable-search">
    <thead class="thead-light">
        <tr>
            <th class="text-uppercase font-weight-bolder opacity-7">File Name</th>
            <th></th>
    ...
    <tbody style="font-size: small">
        ...
        ?>
            <tr>
                <td class="text-center"><?php echo $fileName; ?></td>
                <td>
                    <a href="../../functions/download.php?file=<?php echo urlencode($folder . '/' . $fileName); ?>" class="btn" target="_blank">Download</a>
                </td>
                <td>
                    <button class="btn" onclick="viewFile('<?php echo $folder . '/' . $fileName; ?>')">View</button>
                    <button class="btn" onclick="deleteFile('<?php echo $folder . '/' . $fileName; ?>')">Delete</button>
                </td>
            </tr>
        <?php } ?>
    </tbody>
...
<script >
   function viewFile(fileName) {
    ...
    for (var i = 0; i < allowedPaths.length; i++) {
        var fileUrl = allowedPaths[i] + fileName;  // Remove encodeURIComponent

        // Make an AJAX request to fetch the file content
        $.ajax({
            url: fileUrl,
            type: 'GET',
            dataType: 'text',
            success: function (fileContent) {
                found = true;
                // Display the file content in a modal (you need to implement your own modal logic)
                alert('File Content:\n' + fileContent);
            },
            error: function () {
                // Continue to the next path if there's an error
                if (i === allowedPaths.length - 1 && !found) {
    ...
    function deleteFile(fileName) {
        if (confirm('Are you sure you want to delete ' + fileName + '?')) {
            ...
            $.ajax({
                url: '../../functions/delete.php', // Replace with the path to your delete script
                type: 'POST',
                data: { file: fileName },
...
function downloadFile(fileName) {
    // Implement your logic to download the file
    window.location.href = '../../functions/download.php?file=' + encodeURIComponent(fileName);
}
        ...
        function clearSpace() {
            ...
            $.ajax({
                url: '../../functions/clear_space.php',
                type: 'GET',
                success: function (response) {
                    alert(response); // Display success message
                },
                error: function () {
                    alert('Error clearing space.'); // Display error message
                }
            });
        }
