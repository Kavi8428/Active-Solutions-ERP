...
$query = "SELECT customer_id, company_name, address, type, credit_limit, payment_terms, note, vat FROM customer";
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        ...
        <div class="container-fluid py-4">
            <div style="height: 40px;" class="row">
                ...
                <div class="col-2">
                    <a href="../pages/account/newClient.php">
                        <button class="btn btn-dark">New Customer</button>
                    </a>
                </div>
            </div>
            <div class="row mt-2">
                <div class="col-12">
                            ...
                            <table class="table table-flush" id="datatable-search">
                                <thead class="thead-light">
                                    <tr>
                                        ...
                                        <th class="text-uppercase text-xs font-weight-bolder opacity-7 ps-2">VAT No</th>
                                        <th class="text-uppercase text-xs font-weight-bolder opacity-7 ps-2">Credit
                                            limit</th>
                                        <th class="text-uppercase text-xs font-weight-bolder opacity-7 ps-2">Payment
                                            Term</th>
                                        <th class="text-center text-uppercase text-xs font-weight-bolder opacity-7">
                                            Address</th>
                                        <th class="text-center text-uppercase text-xs font-weight-bolder opacity-7">Note
                                        </th>
                                        <th class="text-center text-uppercase text-xs font-weight-bolder opacity-7">
                                            Action</th>
                                    </tr>
                  ...
                  while ($row = mysqli_fetch_assoc($result)) {
                    echo '<tr>';
                    echo '<td style="font-size:12px";>' . $row['customer_id'] . '</td>';
                    echo '<td style="font-size:12px";>' . $row['type'] . '</td>';
                    echo '<td style="font-size:12px";>' . $row['company_name'] . '</td>';
                    echo '<td style="font-size:12px";>' . $row['vat'] . '</td>';
                    echo '<td style="font-size:12px";>' . $row['credit_limit'] . '</td>';
                    echo '<td style="font-size:12px";>' . $row['payment_terms'] . '</td>';
                    echo '<td style="font-size:12px";>' . $row['address'] . '</td>';
                    echo '<td style="font-size:12px";>' . $row['note'] . '</td>';
                    echo '<td>';
                    echo '<a href="../pages/account/selectedCEDetails.php?customer_id=' . $row['customer_id'] . '"s><button class="btn">EMP</button></a>';
                    echo '<a href="../pages/account/edit_client.php?customer_id=' . $row['customer_id'] . '"><button class="btn">Edit</button></a>';
                    echo '<a href="?delete=true&customer_id=' . $row['customer_id'] . '" onclick="return confirm(\'Are you sure you want to delete this data?\');"><button class="btn">Del</button></a>';
                    echo '</td>';
                    echo '</tr>';
                  }
                  // Check if the delete button was clicked
                  if (isset($_GET['delete']) && isset($_GET['customer_id'])) {
                    $deleteCustomerId = $_GET['customer_id'];
                
                    // First, query the data to be deleted
                    $selectSql = "SELECT * FROM customer WHERE customer_id = $deleteCustomerId";
                    $result = $conn->query($selectSql);
                
                    if ($result->num_rows > 0) {
                        $row = $result->fetch_assoc();
                
                        logEvent($_SESSION['user'], 'Delete customer', 'Customer ID: ' . $row['customer_id'], 1);

                        // Create a backup file in .doc format
                        $backupFileName = 'customer_backup_' . date('Y-m-d_H-i-s') . '.doc';
                        $backupFilePath = '../../assets/customer_backup/' . $backupFileName;
                
                        $backupContent = "Backup of Customer Data\n";
                        $backupContent .= "Customer ID: " . $row['customer_id'] . "\n";
                        $backupContent .= "Type: " . $row['type'] . "\n";
                        $backupContent .= "Company Name: " . $row['company_name'] . "\n";
                        $backupContent .= "Credit Limit: " . $row['credit_limit'] . "\n";
                        $backupContent .= "Payment Terms: " . $row['payment_terms'] . "\n";
                        $backupContent .= "Address: " . $row['address'] . "\n";
                        $backupContent .= "Note: " . $row['note'] . "\n";
                
                        // Save the backup content to the file
                        file_put_contents($backupFilePath, $backupContent);
                
                        // Now, you can safely delete the customer record
                        $deleteCustomerSql = "DELETE FROM customer WHERE customer_id = $deleteCustomerId";
                        ...
                        if ($conn->query($deleteCustomerSql) === TRUE) {
                            ...
                            echo '<div class="alert alert-danger">Error deleting customer record: ' . $conn->error . '</div>';
                            echo '<script>window.location.reload();</script>';
