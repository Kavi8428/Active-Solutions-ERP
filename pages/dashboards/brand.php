    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        ...
        <nav class="navbar navbar-main navbar-expand-lg position-sticky mt-4 top-1 px-0 mx-4 shadow-none border-radius-xl z-index-sticky"
            ...
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    ...
                    <h6 class="font-weight-bolder mb-0">Brand Details</h6>
                </nav>
        ...
        <div class="container-fluid py-4">
            <div style="height: 40px;" class="row">
                <div class="col-10">
                    <h5 class="mb-0">BrandDetails</h5>
                </div>
                <div class="col-2">
                    <a href="../pages/account/new_brand.php">
                        <button class="btn btn-dark">New Brand</button>
                    </a>
                </div>
            </div>
            <div class="row mt-2">
                        ...
                        <table class="table table-flush" id="datatable-search">
                                <thead class="thead-light">
                                    <tr>
                                        ...
                                        <th class="text-uppercase text-xxs font-weight-bolder opacity-7 ps-2">Brand</th>
...
while ($row = mysqli_fetch_assoc($result)) {
    echo '<tr>';
    echo '<td style="font-size:12px;">' . $row['brand_id'] . '</td>';
    echo '<td style="font-size:12px;">' . $row['brand'] . '</td>';
    echo '<td>';
    echo '<a href="../pages/account/edit_brand.php?brand_id=' . $row['brand_id'] . '"><button class="btn">Edit</button></a>';
    echo '<a href="?delete=true&brand_id=' . $row['brand_id'] . '" onclick="return confirm(\'Are you sure you want to delete this data?\');"><button class="btn">Del</button></a>';
    echo '</td>';
    echo '</tr>';
}

// Check if the delete button was clicked
if (isset($_GET['delete']) && isset($_GET['brand_id'])) {
    $deleteCustomerId = $_GET['brand_id'];

    // Use a prepared statement to prevent SQL injection
    $selectSql = "SELECT * FROM brand   WHERE brand_id = ?";
    $stmt = $conn->prepare($selectSql);
    $stmt->bind_param('i', $deleteCustomerId);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        logEvent($_SESSION['user'], 'Delete Brand', 'Brand ID: ' . $row['brand_id'], 1);
        // Create a backup file in .doc format
        $backupFileName = 'brand_backup_' . date('Y-m-d_H-i-s') . '.doc';
        $backupFilePath = '../../assets/category_backup/' . $backupFileName;

        $backupContent = "Backup of Category Data\n";
        $backupContent .= "Brand ID: " . $row['brand_id'] . "\n";
        $backupContent .= "Brand: " . $row['brand'] . "\n";
        ...
        $deleteCustomerSql = "DELETE FROM brand WHERE brand_id = ?";
        ...
        if ($stmt->execute()) {
            ...
            echo '<div class="alert alert-danger">Error deleting category record: ' . $stmt->error . '</div>';
            echo '<script>window.location.reload();</script>';
        }
    } else {
