...
if (isset($_GET['item_code'])) {
    ...
    // Fetch the product data for the provided "item_code" from the database
    ...
    if ($result->num_rows > 0) {
        ...
        // Product not found, handle the error or redirect back to the product list page.
        echo "Product not found.";
    }
} else {
    // "item_code" parameter not provided in the URL, handle the error.
...
if (isset($_GET['item_code'])) {
    ...
    // Create and execute a SQL query to fetch the product data based on the item_code
    $query = "SELECT * FROM internal_drive WHERE item_code = '$item_code'";
...
if (isset($_GET['item_code'])) {
    ...
    // Create and execute a SQL query to fetch the product data based on the item_code
    $query = "SELECT * FROM external_drive WHERE item_code = '$item_code'";
    $result = mysqli_query($conn, $query);
...
if (isset($_GET['item_code'])) {
  $item_code = $_GET['item_code'];
        ...
        // Now use $merged_id in your next query
        $sql1 = "SELECT * FROM product_synology WHERE item_code = '$item_code'";
  ...
  if ($result1->num_rows > 0) {
    ...
    $cpuQt = $row['cpuQt'];
    $cpuFrequency = $row['cpuFrequency'];
    $systemMemory = $row['systemMemory'];
    $cpuArchitecture = $row['cpuArchitecture'];
    $totalMemorySlot = $row['totalMemorySlot'];
    $memoryModule = $row['memoryModule'];
    $maximumMemoryCapacity = $row['maximumMemoryCapacity'];
    $compatibleDriveType = $row['compatibleDriveType'];
    $driveBays = $row['driveBays'];
    $maximumExpansionUnits = $row['maximumExpansionUnits'];
    $driveSlots = $row['driveSlots'];
    $maxBayWithExpanUnit = $row['maxBayWithExpanUnit'];
    $MaximumLocalUserAccounts = $row['MaximumLocalUserAccounts'];
    $RJ_45_1GbE = $row['RJ_45_1GbE'];
    $RJ_45_10GbE = $row['RJ_45_10GbE'];
    $RJ_45_25GbEt = $row['RJ_45_25GbEt'];
    $usbPort = $row['usbPort'];
    $usbCopy = $row['usbCopy'];
    $eSataPort = $row['eSataPort'];
    $highAvailabilty = $row['highAvailabilty'];
    $maximumConcurrentConnection = $row['maximumConcurrentConnection'];
    $maximumConcurrentConnectionWithRam = $row['maximumConcurrentConnectionWithRam'];
    $PoolSupport = $row['PoolSupport'];
    $readWriteCashe = $row['readWriteCashe'];
    $sysLogEventPerSecond = $row['sysLogEventPerSecond'];
    $maximumIpCam = $row['maximumIpCam'];
    $warrenty = $row['warrenty'];
} else {
    // Product not found, handle the error or redirect back to the product list page.
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  $totalMemorySlot = $_POST['totalMemorySlot'];
  $memoryModule = $_POST['memoryModule'];
  $maximumMemoryCapacity = $_POST['maximumMemoryCapacity'];
  $compatibleDriveType = $_POST['compatibleDriveType'];
  ...
  $sql = "UPDATE product_synology 
          SET MaximumLocalUserAccounts = ?, RJ_45_1GbE = ?, RJ_45_10GbE = ?, RJ_45_25GbEt = ?, usbPort = ?, usbCopy = ?, eSataPort = ?, highAvailabilty = ?, maximumConcurrentConnection = ?, maximumConcurrentConnectionWithRam = ?,  PoolSupport = ?, readWriteCashe = ?, sysLogEventPerSecond = ?, maximumIpCam = ?, cpuModel = ?, cpuQt = ?, cpuFrequency = ?, systemMemory = ?, cpuArchitecture = ?, totalMemorySlot = ?, memoryModule = ?, maximumMemoryCapacity = ?, compatibleDriveType = ?, driveBays = ?, maximumExpansionUnits = ?, driveSlots = ?, maxBayWithExpanUnit = ?
          WHERE id = ?";
         
  $stmt = $conn->prepare($sql);
  $stmt->bind_param("ssiiississsssssssssiiiisssii", $MaximumLocalUserAccounts, $RJ_45_1GbE, $RJ_45_10GbE, $RJ_45_25GbEt, $usbPort, $usbCopy, $eSataPort, $highAvailabilty, $maximumConcurrentConnection, $maximumConcurrentConnectionWithRam, $PoolSupport, $readWriteCashe, $sysLogEventPerSecond, $maximumIpCam, $cpuModel, $cpuQt, $cpuFrequency, $systemMemory, $cpuArchitecture, $totalMemorySlot, $memoryModule, $maximumMemoryCapacity, $compatibleDriveType, $driveBays, $maximumExpansionUnits, $driveSlots, $maxBayWithExpanUnit, $id);
 
  if ($stmt->execute()) {
     $message ="Form updated success";
     $logSql = "INSERT INTO log (user, task, date_time, other, serverity) VALUES (?, ?, NOW(), ?, ?)";
     $logStmt = $conn->prepare($logSql);
 
     $user = $_SESSION['user'];
         $task = 'Edit Product';
         $other = ' Item Code ' . $item_code ;
         $severity = 3; // 1 for informational, adjust as needed
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $ext4 = isset($_POST["in_ext4"]) ? $_POST["in_ext4"] : "no"; // Default to "no" if not checked
    $btrfs = isset($_POST["in_btrfs"]) ? $_POST["in_btrfs"] : "no"; // Default to "no" if not checked

    // Assuming you have an item_code
     // Replace with the actual item code

    // Prepare and execute the SQL statement to update data
    $sql = "UPDATE internal_drive SET ext4 = ?, btrfs = ? WHERE item_code = ?";
    ...
    if ($stmt) {
        // Bind the values
        mysqli_stmt_bind_param($stmt, "sss", $ext4, $btrfs, $item_code);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Get user-selected data
    $fat = isset($_POST["fat"]) ? $_POST["fat"] : "no"; // Default to "no" if not checked
    $btrfs = isset($_POST["btrfs"]) ? $_POST["btrfs"] : "no"; // Default to "no" if not checked
    $ext3 = isset($_POST["ext3"]) ? $_POST["ext3"] : "no"; // Default to "no" if not checked
    $ext4 = isset($_POST["ext4"]) ? $_POST["ext4"] : "no"; // Default to "no" if not checked
    $ntfs = isset($_POST["ntfs"]) ? $_POST["ntfs"] : "no"; // Default to "no" if not checked
    $hfs = isset($_POST["hfs"]) ? $_POST["hfs"] : "no"; // Default to "no" if not checked

    // Assuming you have an item_code
   

    // Prepare and execute the SQL statement to update data
    $sql = "UPDATE external_drive SET fat = ?, btrfs = ?, ext3 = ?, ext4 = ?, ntfs = ?, hfs = ? WHERE item_code = ?";
    ...
    if ($stmt) {
        // Bind the values
        mysqli_stmt_bind_param($stmt, "sssssss", $fat, $btrfs, $ext3, $ext4, $ntfs, $hfs, $item_code);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!empty($_POST["token"]) && hash_equals($_SESSION["token"], $_POST["token"])) {
      ...
      $short_des = isset($_POST["short_desc"]) ? $_POST["short_desc"] : "";
      $long_des = isset($_POST["long_desc"]) ? $_POST["long_desc"] : "";
      $category = $_POST["choices-category"];
      $brand = $_POST["synology"];
      $warrenty = $_POST["warrenty"];

   

      $uploadDir = "../../../assets/product_image/"; // Specify the directory where you want to store uploaded files
      $image1Path = null;
      $image2Path = null;

      if (!empty($_FILES["image1"]["name"])) {
          $image1Path = $uploadDir . $_FILES["image1"]["name"];
          move_uploaded_file($_FILES["image1"]["tmp_name"], $image1Path);
      }

      if (!empty($_FILES["image2"]["name"])) {
          $image2Path = $uploadDir . $_FILES["image2"]["name"];
          move_uploaded_file($_FILES["image2"]["tmp_name"], $image2Path);
      }

      // Prepare and execute the SQL query to update only edited data
      $sql = "UPDATE product SET 
              manufacturer = COALESCE(?, manufacturer),
              short_des = COALESCE(?, short_des),
              long_des = COALESCE(?, long_des),
              category = COALESCE(?, category),
              image1 = COALESCE(?, image1),
              image2 = COALESCE(?, image2),
              brand = COALESCE(?, brand),
              warrenty = COALESCE(?, warrenty)

              WHERE item_code = ?";

      $stmt = $conn->prepare($sql);

      // Bind all parameters corresponding to the placeholders
      $stmt->bind_param("sssssssss", $manufacturer, $short_des, $long_des, $category, $image1Path, $image2Path, $brand, $warrenty, $item_code);
      ...
      echo "Token mismatch. Form resubmission prevented.";
  }
} else {
...
<body class="g-sidenav-show  bg-gray-200">
            ...
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm text-dark active" aria-current="page"> <a href="../../ecommerce/products/products-list.php" class="lin">Products</a> </li>
                    </ol>
                    <h6 class="font-weight-bolder mb-0">NAS</h6>
                </nav>
        ...
        <div class="container-fluid py-4">
            <div class="row min-vh-80">
                <div class="col-lg-8 col-md-10 col-12 m-auto">
                    <h3 class="mt-3 mb-0 text-center">Edit Synology</h3>
                    <div class="card">
                                    ...
                                    <div class="multisteps-form__content">
                                        <div class="row mt-3">
                                            <div class="col-12 col-sm-6">
                                                <div class="input-group input-group-dynamic">
                                                    ...
                                                    <input class="multisteps-form__input form-control" type="text"
                                                        name="item_code" disabled value=<?php echo $item_code?> />
                                                </div>
                                            </div>
                                            <div class="col-12 col-sm-6 mt-3 mt-sm-0">
                                                <div class="input-group input-group-dynamic">
                                                    ...
                                                    <input class="multisteps-form__input form-control" type="text"
                                                        name="manufacturer" value=<?php echo $manufacturer?> />
                                        ...
                                        <div class="row">
                                                ...
                                                <div class="multisteps-form__content">
                                                    <div class="row mt-3">
                                                        <textarea name="short_desc"
                                                            style="width: 650px; height: 140px; padding: 10px; font-size: 16px; border: 1px solid #ccc;"
                                                           ><?php echo $short_des?></textarea>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-sm-6 mt-sm-3 mt-5">
                                                ...
                                                <input class="multisteps-form__input form-control" type="text"
                                                    name="choices-category" value="<?php echo $category?>" />

                                                <label class="form-control ms-0">Brand</label>
                                                <input class="multisteps-form__input form-control" type="text"
                                                    name="synology" value="Synology" />
                                            ...
                                            <div class="row mt-2">
                                                ...
                                                <div class="col-6">
                                                    ...
                                                    <input type="file" name="img1" placeholder="Click to upload image 1"
                                                        value=<?php echo $image1?>><br>
                                                    <input type="file" name="img2" placeholder="Click to upload image 2"
                                                        value=<?php echo $image2?>><br>
                                                </div>
                                                <div class="col-6 mt-3 mt-sm-0">
                                                <div class="input-group input-group-dynamic">
                                                    <label  class="form-label mb-3 mt-3 " for="warrenty"
                                                       >Warrenty</label>
                                                    <input class="multisteps-form__input form-control mt-2" type="text"
                                                        name="warrenty" id="warrenty" value=<?php echo $warrenty?> />
                                            ...
                                            <div class="row mt-1">
                                                <div class="button-row d-flex mt-7">
                                                    ...
                                                    <div class="button-row d-flex mt-1 ms-auto">
                                                        <a href="../products/bdcom.php"
                                                            class="btn bg-gradient-dark  mb-0" type="button"
                                                            title="Next">If BD Com</a>
                                                    </div>
                                        ...
                                        <div class="row mt-3">
                                            <div class="col-12">
                                                <textarea name="long_desc"
                                                    style="width: 650px; height: 150px; padding: 10px; font-size: 16px; border: 1px solid #ccc;"
                                                    ><?php echo $long_des?></textarea>
                                            </div>
                                        </div>
                                    ...
                                    <div class="multisteps-form__content mt-0">
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-2">
                                                            <div class="input-group input-group-dynamic">
                                                                <div class="form-check">
                                                                    <input class="form-check-input" type="radio"
                                                                        name="cpuQt" id="cpuqt2" value="2"
                                                                        <?php if($cpuQt == '2'){echo "checked";}?>>
                                                                    <label class="form-check-label"
                                                                        for="cpuqt2">2</label>
                                                                </div>
                                                            </div>
                                                ...
                                                <div class="input-group input-group-dynamic">
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio"
                                                            ...
                                                            <?php if($cpuArchitecture=='0'){echo 'checked';}?>>
                                                        <label class="form-check-label" for="0">0</label>
                                                    </div>
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio"
                                                            name="cpuArchitecture" id="32" value="32"
                                                            <?php if($cpuArchitecture=='32'){echo 'checked';}?>>
                                                        <label class="form-check-label" for="32">32</label>
                                                    </div>
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio"
                                                            name="cpuArchitecture" id="64" value="64"
                                                            <?php if($cpuArchitecture=='64'){echo 'checked';}?>>
                                        ...
                                        <div class="row mt-4">
                                            ...
                                            <div class="col-9">
                                                <div class="input-group input-group-dynamic">
                                                    ...
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio"
                                                            name="totalMemorySlot" id="tms0" value="0"
                                                            <?php if($totalMemorySlot=='0'){echo 'checked';}?>>
                                                    ...
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio"
                                                            ...
                                                            <?php if($totalMemorySlot=='1'){echo 'checked';}?>>
                                                    ...
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio"
                                                            ...
                                                            <?php if($totalMemorySlot=='2'){echo 'checked';}?>>
                                        ...
                                        <div class="row mt-2">
                                            ...
                                            <div class="col-3">
                                                <div class="input-group input-group-dynamic">
                                                    <input class="multisteps-form__input form-control" type="text"
                                                        name="memoryModule" value="<?php echo $memoryModule?>" />
                                                </div>
                                            </div>
                                            <div class="col-4">
                                                <div class="input-group input-group-dynamic">
                                            ...
                                            <div class="col-2">
                                                <div class="input-group input-group-dynamic">
                                                    <input class="multisteps-form__input form-control" type="text"
                                                        ...
                                                        value=" <?php echo $maximumMemoryCapacity?> " />
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row mt-3">
                                            ...
                                            <div class="col-9">
                                                <div class="input-group input-group-dynamic">
                                                    ...
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio"
                                                            name="compatibleDriveType" id="SAS" value="0"
                                                            <?php if($compatibleDriveType== 0){echo 'checked';};?>>
                                                    ...
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio"
                                                            ...
                                                            <?php if($compatibleDriveType== 1){echo 'checked';}?>>
                                        ...
                                        <div class="row mt-2">
                                            ...
                                            <div class="col-3">
                                                <div class="input-group input-group-dynamic">
                                                    <input class="multisteps-form__input form-control" type="text"
                                                        name="driveBays" value="<?php echo $driveBays?> " />
                                                </div>
                                            </div>
                                            <div class="col-4">
                                                <div class="input-group input-group-dynamic">
                                            ...
                                            <div class="col-2">
                                                <div class="input-group input-group-dynamic">
                                                    <input class="multisteps-form__input form-control" type="text"
                                                        ...
                                                        value="<?php echo $maximumExpansionUnits?>" />
                                                </div>
                                            </div>
                                        </div>
                                                ...
                                                <div class="input-group input-group-dynamic">
                                                    ...
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio" name="driveSlots"
                                                            id="m20" value="0"
                                                            <?php if($driveSlots=='0'){echo 'checked';}?>>
                                            ...
                                            <div class="col-2">
                                                <div class="input-group input-group-dynamic">
                                                    ...
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio" name="driveSlots"
                                                            id="m22" value="2"
                                                            <?php if($driveSlots=='2'){echo 'checked';}?>>
                                ...
                                <div class="multisteps-form__panel pt-3 border-radius-xl bg-white h-150"
                                    ...
                                    <div class="multisteps-form__content mt-3">
                                        <div class="row mt-3">
                                            <div class="col-5">
                                                <div class="input-group input-group-dynamic">
                                                    <label class="form-label"> <b>Maximum Local User Accounts</b>
                                                    </label>
                                                </div>
                                            </div>
                                            <div class="col-7">
                                                <div class="input-group input-group-dynamic">
                                                    <input class="multisteps-form__input form-control" type="text"
                                                        name="MaximumLocalUserAccounts" placeholder=" MaximumLocalUser"
                                                        value="<?php echo $MaximumLocalUserAccounts?>" />
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row mt-3">
                                            ...
                                            <div class="col-3">
                                                <div class="input-group input-group-dynamic">
                                                    <select class="multisteps-form__input form-control"
                                                        ...
                                                        <option <?php  if($RJ_45_1GbE=="" ){echo 'selected';}?>>RJ-45
                                                            1GbE</option>
                                                        <option value="1"
                                                            <?php if($RJ_45_1GbE=='1'){echo 'selected';}?>>1</option>
                                                        <option value="2"
                                                            <?php  if($RJ_45_1GbE=="2" ){echo 'selected';}?>>2</option>
                                                        <option value="3"
                                                            <?php  if($RJ_45_1GbE=="3" ){echo 'selected';}?>>3</option>
                                                        <option value="4"
                                                            <?php  if($RJ_45_1GbE=="4" ){echo 'selected';}?>>4</option>
                                                        <option value="8"
                                                            <?php  if($RJ_45_1GbE=="8" ){echo 'selected';}?>>8</option>
                                                    </select>
                                                </div>
                                            </div>
                                            <div class="col-3">
                                                <select class="form-control" name="RJ_45_10GbE" id="choices-sizes">
                                                    <option <?php  if($RJ_45_10GbE=="" ){echo 'selected';}?>>RJ-45 10GbE
                                                    </option>
                                                    <option value="0" <?php  if($RJ_45_10GbE=="0" ){echo 'selected';}?>>
                                                        0</option>
                                                    <option value="1" <?php  if($RJ_45_10GbE=="1" ){echo 'selected';}?>>
                                                        1</option>
                                                    <option value="2" <?php  if($RJ_45_10GbE=="2" ){echo 'selected';}?>>
                                                        2</option>

                                                </select>
                                            </div>
                                            <div class="col-3">
                                                <select class="form-control" name="RJ_45_25GbEt" id="choices-lan1">
                                                    <option <?php  if($RJ_45_25GbE=="" ){echo 'selected';}?>>RJ-45
                                                        25GbEt</option>
                                                    <option value="0"
                                                        <?php  if($RJ_45_25GbEt=="0" ){echo 'selected';}?>>0</option>
                                                    <option value="1"
                                                        <?php  if($RJ_45_25GbEt=="1" ){echo 'selected';}?>>1</option>
                                                    <option value="2"
                                                        <?php  if($RJ_45_25GbEt=="2" ){echo 'selected';}?>>2</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="row mt-3">
                                            ...
                                            <div class="col-3">
                                                <div class="input-group input-group-dynamic">
                                                    <select class="multisteps-form__input form-control" name="usbPort"
                                                        id="choices-usb1" required>

                                                        <option <?php  if($usbPort=="" ){echo 'selected';}?>>USB 3.2 Gen
                                                            1</option>
                                                        <option value="0" <?php  if($usbPort=="0" ){echo 'selected';}?>>
                                                            0</option>
                                                        <option value="1" <?php  if($usbPort=="1" ){echo 'selected';}?>>
                                                            1</option>
                                                        <option value="2" <?php  if($usbPort=="2" ){echo 'selected';}?>>
                                                            2</option>
                                                        <option value="3" <?php  if($usbPort=="3" ){echo 'selected';}?>>
                                                            3</option>
                                                        <option value="4" <?php  if($usbPort=="4" ){echo 'selected';}?>>
                                                            4</option>
                                                    </select>
                                                </div>
                                            </div>
                                            ...
                                            <div class="col-3">
                                                <select class="multisteps-form__input form-control" name="usbCopy"
                                                    id="choices-usbCopy">
                                                    <option value="Yes" <?php  if($usbCopy=="Yes" ){echo 'selected';}?>>
                                                        Yes</option>
                                                    <option value="No" <?php  if($usbCopy=="No" ){echo 'selected';}?>>NO
                                                    </option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="row mt-3">
                                            ...
                                            <div class="col-3">
                                                <select class="multisteps-form__input form-control"
                                                    name="highAvailabilty" id="choices-highAvailability">
                                                    <option value="Yes"
                                                        <?php  if($highAvailabilty=="Yes" ){echo 'selected';}?>>Yes
                                                    </option>
                                                    <option value="No"
                                                        <?php  if($highAvailabilty=="No" ){echo 'selected';}?>>NO
                                                    </option>
                                                </select>
                                            </div>
                                        </div>
                                            ...
                                            <div class="col-7">
                                                <div class="input-group input-group-dynamic">
                                                    <input class="multisteps-form__input form-control" type="text"
                                                        ...
                                                        value="<?php echo $maximumConcurrentConnection?>" />
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row mt-3">
                                            <div class="col-5">
                                                <div class="input-group input-group-dynamic">
                                                    <label class="form-label"><b>Maximum Concurrent Connections (with
                                                            RAM expansion):</b>
                                                    </label>
                                                </div>
                                            </div>
                                            <div class="col-7">
                                                <div class="input-group input-group-dynamic">
                                                    <input class="multisteps-form__input form-control" type="text"
                                                        ...
                                                        value="<?php echo $maximumConcurrentConnectionWithRam?>" />
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row mt-4">
                                        <div class="col-5">
                                                        <label><b>Internal Drives</b></label><br>
                                                    </div>
                                                    <div class="col-4">
                                                        <div class="form-check">
                                                            <input type="checkbox" class="form-check-input"
                                                                id="internalDrive-ext4" name="in_ext4" value="EXT4"
                                                                <?php  if($row3['ext4'] == 'EXT4'){echo 'checked';}?>>
                                                            <label class="form-check-label"
                                                    ...
                                                    <div class="col-3">
                                                        <div class="form-check">
                                                            <input type="checkbox" class="form-check-input"
                                                                id="internalDrive-btrfs" name="in_btrfs" value="Btrfs"
                                                                <?php  if($row3['btrfs'] == 'Btrfs'){echo 'checked';}?>>
                                                            <label class="form-check-label"
                                        ...
                                        <div class="row mt-4">
                                            <div class="col-3">
                                                <label><b>External Drives</b></label><br>
                                            </div>
                                            <div class="col-9">
                                                <div class="row">
                                                    <div class="col-2">
                                                        <div class="form-check">
                                                            <input type="checkbox" class="form-check-input"
                                                                ...
                                                                <?php if(isset($row4['fat']) && $row4['fat'] == 'FAT') echo 'checked'; ?>>
                                                            <label class="form-check-label"
                                                    ...
                                                    <div class="col-2">
                                                        <div class="form-check">
                                                            <input type="checkbox" class="form-check-input"
                                                                ...
                                                                <?php if(isset($row4['btrfs']) && $row4['btrfs'] == 'Btrfs') echo 'checked'; ?>>
                                                            <label class="form-check-label"
                                                    ...
                                                    <div class="col-2">
                                                        <div class="form-check">
                                                            <input type="checkbox" class="form-check-input"
                                                                ...
                                                                <?php if(isset($row4['ext3']) && $row4['ext3'] == 'Btrfs') echo 'checked'; ?>>
                                                            <label class="form-check-label"
                                                    ...
                                                    <div class="col-2">
                                                        <div class="form-check">
                                                            <input type="checkbox" class="form-check-input"
                                                                id="externalDrive-ext4" name="ext4" value="EXT4" <?php if(isset($row4['ext4']) && $row4['ext4'] == 'EXT4') echo 'checked'; ?>>
                                                            <label class="form-check-label"
                                                    ...
                                                    <div class="col-2">
                                                        <div class="form-check">
                                                            <input type="checkbox" class="form-check-input"
                                                                id="externalDrive-ext5" name="ntfs" value="NTFS" <?php if(isset($row4['ntfs']) && $row4['ntfs'] == 'NTFS') echo 'checked'; ?>>
                                                            <label class="form-check-label"
                                                    ...
                                                    <div class="col-2">
                                                        <div class="form-check">
                                                            <input type="checkbox" class="form-check-input"
                                                                id="externalDrive-ext6" name="hfs" value="HFS+"<?php if(isset($row4['hfs']) && $row4['hfs'] == 'HFS+') echo 'checked'; ?>>
                                                            <label class="form-check-label"
                                        ...
                                        <div class="row mt-3">
                                            ...
                                            <div class="col-3">
                                                <select class="multisteps-form__input form-control" name="PoolSupport"
                                                    id="PoolSupport">
                                                    <option value="Yes"
                                                        <?php  if($PoolSupport=="Yes" ){echo 'selected';}?>>Yes</option>
                                                    <option value="No"
                                                        <?php  if($PoolSupport=="No" ){echo 'selected';}?>>No</option>
                                                </select>
                                            </div>
                                            <div class="col-3">
                                                <div class="input-group input-group-dynamic">
                                                    <label class="form-label"><b>Read / Write Cache (White Paper) :</b>
                                                    </label>
                                                </div>
                                            </div>
                                            <div class="col-3">
                                                <select class="multisteps-form__input form-control"
                                                    name="readWriteCashe" id="SSDReadWrite">
                                                    <option value="Yes"
                                                        <?php  if($readWriteCashe=="Yes" ){echo 'selected';}?>>Yes
                                                    </option>
                                                    <option value="No"
                                                        <?php  if( $readWriteCashe=="No" ){echo 'selected';}?>>No
                                                    </option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="row mt-3">
                                            ...
                                            <div class="col-7">
                                                <div class="input-group input-group-dynamic">
                                                    <input class="multisteps-form__input form-control" type="text"
                                                        name="sysLogEventPerSecond"
                                                        value="<?php echo $sysLogEventPerSecond?>" />
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row mt-3">
                                            <div class="col-5">
                                                <div class="input-group input-group-dynamic">
                                                    <label class="form-label"><b>Maximum IP cam (Licenses required)
                                                            :</b>
                                                    </label>
                                                </div>
                                            </div>
                                            <div class="col-7">
                                                <div class="input-group input-group-dynamic">
                                                    <input class="multisteps-form__input form-control" type="text"
                                                        name="maximumIpCam" value="<?php echo $maximumIpCam?>" />
                                                </div>
                                            </div>
                                        </div>
                                         <div class="row mt-3">
    ...
    if (document.getElementById('ExternalDrives')) {
        var element = document.getElementById('ExternalDrives');
        const example = new Choices(element, {
            searchEnabled: false
        });
    };
