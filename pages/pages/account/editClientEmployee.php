...
// Check if the "cus_em_id" is provided in the URL
if (isset($_GET['id'])) {
    $cus_em_id = $_GET['id'];

    // Fetch the employee data for the provided "cus_em_id" from the database
    $sql = "SELECT * FROM customer_employee WHERE cus_em_id = '$cus_em_id'";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        // Fetch and store employee details
        $cus_em_id = $row['cus_em_id'];
        $cus_em_name = $row['cus_em_name'];
        $dob = $row['dob'];
        $tel1 = $row['tel1'];
        $tel2 = $row['tel2'];
        $type = $row['type'];
        ...
        // Employee not found, handle the error or redirect back to the employee list page.
        echo "Employee not found.";
    }
} else {
    // "cus_em_id" parameter not provided in the URL, handle the error.
    echo "cus_em_id not provided.";
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!empty($_POST["token"]) && hash_equals($_SESSION["token"], $_POST["token"])) {
    ...
    $cus_em_id = $_POST["cus_em_id"];
    $cus_em_name = $_POST["cus_em_name"];
    $dob = $_POST["dob"];
    $email = $_POST["email"];
    $tel1 = $_POST["tel1"];
    $tel2 = $_POST["tel2"];
    ...
    // Check that the cus_em_id variable is not empty
    if (empty($cus_em_id)) {
      echo "The cus_em_id variable is empty.";
      exit;
    }

    // Prepare and execute the SQL query to update only edited data
    $sql = "UPDATE customer_employee SET
      cus_em_name = COALESCE(?, cus_em_name),
      dob = COALESCE(?, dob),
    ...
    $stmt->bind_param("ssssssi", $cus_em_name, $dob, $email, $tel1, $tel2, $type, $cus_em_id);
    ...
    if ($stmt->execute()) {
      ...
      $user = $_SESSION['user'];
          $task = 'Edit Customer Employee';
          $other = 'Edited CE Id : '.$cus_em_id ;
...
<!-- Add your HTML form here and pre-populate the form fields with $address and $note -->
        ...
        <nav class="navbar navbar-main navbar-expand-lg position-sticky mt-4 top-1 px-0 mx-4 shadow-none border-radius-xl z-index-sticky"
            ...
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    ...
                    <h6 class="font-weight-bolder mb-0">Edit CE</h6>
                </nav>
                ...
                <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4" id="navbar">
                    ...
                    <ul class="navbar-nav  justify-content-end">
                        <li class="nav-item">
                            <a href="../profile/overview" class="nav-link text-body p-0 position-relative"
                                target="_blank">
                                        ...
                                        <div class="bg-dark shadow-dark border-radius-lg pt-4 pb-3">
                                            <div class="multisteps-form__progress">
                                                ...
                                                <button class="multisteps-form__progress-btn" type="button"
                                                    title="Address">Addresss</button>
                                                <button class="multisteps-form__progress-btn" type="button"
                                                    title="Socials">Notes</button>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="card-body">
                                        <form class="multisteps-form__form" method="POST" enctype="multipart/form-data">
                                            ...
                                            <div class="multisteps-form__panel border-radius-xl bg-white js-active"
                                                ...
                                                <h5 class="font-weight-bolder mb-0">Customer Employee</h5>
                                                <p class="mb-0 text-sm">Edit Customer Employee details</p>
                                                <div class="multisteps-form__content">
                                                    <div class="row mt-3">
                                                        <div class="col-12 col-sm-6 mt-3 mt-sm-0">
                                                            <div class="input-group input-group-dynamic">
                                                                <label class="form-label">CE Name</label>
                                                                <input class="multisteps-form__input form-control"
                                                                    type="text" name="cus_em_name"
                                                                    value="<?php echo $row['cus_em_name']; ?>" />

                                                            </div>
                                                        </div>
                                                        <input type="hidden" name="cus_em_id"
                                                            value="<?php echo $row['cus_em_id']; ?>">
                                                    ...
                                                    <div class="row mt-3">
                                                        <div class="col-12 col-sm-6">
                                                            <div class="input-group input-group-dynamic">
                                                                <label class="form-label">Tel 01</label>
                                                                <input class="multisteps-form__input form-control"
                                                                    type="text" name="tel1"
                                                                    value="<?php echo $row['tel1']; ?>" />
                                                            </div>
                                                        </div>
                                                        <div class="col-12 col-sm-6 mt-3 mt-sm-0">
                                                            <div class="input-group input-group-dynamic">
                                                                <label class="form-label">Tel 02</label>
                                                                <input class="multisteps-form__input form-control"
                                                                    type="text" name="tel2"
                                                                    value="<?php echo $row['tel2']; ?>" />
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="row mt-3">
                                                        <div class="col-12 col-sm-6">
                                                            <div class="input-group input-group-dynamic">
                                                                <select class="form-control" name="type"
                                                                    id="choices-questions" required>

                                                                    <option value="sales">
                                                                        &nbsp;<?php echo $row['type']; ?></option>

                                                                    <option value="presales">&nbsp;Presales
                                                                    </option>
...
// Query to retrieve the company name based on cus_em_id
$companyQuery = "SELECT c.company_name
             FROM customer c
             JOIN cus_emp_junc j ON c.customer_id = j.customer_id
             WHERE j.cus_em_id = $cus_em_id";
...
if ($companyResult && $companyRow = mysqli_fetch_assoc($companyResult)) {
  ...
  $company_name = ''; // Default value if no company name is found
}
...
if ($result) {
  echo '<select class="form-control" name="company" id="choices-questions" required>';
  echo '<option>'. $company_name.'</option>';

  // Loop through the results and create an option for each company
  while ($companyRow = mysqli_fetch_assoc($result)) {
    $companyName = $companyRow['company_name']; // Use 'company_name' column name
    echo '<option value="' . $companyName . '">&nbsp;' . $companyName . '</option>';
  }

  echo '</select>';
} else {
  echo 'Error: ' . mysqli_error($conn); // Use $conn here to display the error
}
                                                        ...
                                                        <div class="col-12 ">
                                                            <div class="input-group input-group-dynamic">
                                                                <input class="multisteps-form__textarea form-control"
                                                                    placeholder="Say a few words about the employee"
                                                                    name="note" value="<?php echo $row['note']; ?>" />
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="button-row d-flex mt-4">
                                                        <button class="btn bg-gradient-light mb-0 js-btn-prev"
                                                            type="button" title="Prev">Prev</button>
                                                        <button class="btn bg-gradient-dark ms-auto mb-0 js-btn-next"
                                                            type="submit" value="submit" title="Send">Next</button>
                    ...
                    <div class="row align-items-center justify-content-lg-between">
                        ...
                        <div class="col-lg-6">
                            <ul class="nav nav-footer justify-content-center justify-content-lg-end">
                                ...
                                <li class="nav-item">
                                    <a href="https://www.activelkcom" class="nav-link text-muted"
