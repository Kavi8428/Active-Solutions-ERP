                    ...
                    <div class="card">
                        ...
                        <div class="card-body px-0 pb-0">
                            <div class="table-responsive">
                                <table class="table table-flush" id="products-list" searchable>
                                            ...
                                            <th class="text-uppercase text-xxs font-weight-bolder opacity-7 ps-3">
                                                <div>
                                                    ...
                                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                        <li><a class a="dropdown-item" href="#">Partner</a></li>
                                                        <li><a class a="dropdown-item" href="#">End Customer</a></li>
                                                    </ul>
                                                </div>
                                            </th>
                                            <th class="text-uppercase text-xs font-weight-bolder opacity-7 ps-2">Name
                                            </th>
                                            <th class="text-uppercase text-xs font-weight-bolder opacity-7 ps-2">DOB
                                            </th>
                                            <th class="text-uppercase text-xs font-weight-bolder opacity-7 ps-2">E-Mail
                                            </th>
                                            <th class="text-uppercase text-xs font-weight-bolder opacity-7 ps-2">Tel 1
                                            </th>
                                            <th class="text-uppercase text-xs font-weight-bolder opacity-7 ps-2">Tel 2
                                            </th>
...
if (isset($_GET['customer_id'])) {
    ...
    // Construct the SQL query to retrieve all cus_em_id values related to the customer_id
    $cus_em_id_query = "SELECT cus_em_id FROM cus_emp_junc WHERE customer_id = '$customer_id'";

    // Execute the SQL query to get all cus_em_id values
    $cus_em_id_result = mysqli_query($conn, $cus_em_id_query);

    // Check if the query was successful
    if ($cus_em_id_result) {
        // Initialize an array to store all cus_em_id values
        $cus_em_ids = array();

        // Fetch all cus_em_id values and store them in the array
        while ($row = mysqli_fetch_assoc($cus_em_id_result)) {
            $cus_em_ids[] = $row['cus_em_id'];
        }

        // Initialize an array to store records
        $records = array();

        // Loop through the cus_em_ids and retrieve and store the corresponding records
        foreach ($cus_em_ids as $cus_em_id) {
            // Construct the SQL query to retrieve customer_employee data using cus_em_id
            $query = "SELECT * FROM customer_employee WHERE cus_em_id = '$cus_em_id'";

            // Execute the SQL query to get customer_employee data
            ...
            if ($res) {
                ...
                if (mysqli_num_rows($res) > 0) {
                    // Fetch the customer data and store it in the $records array
        ...
        foreach ($records as $data) {
            echo '<tr>';
            echo '<td><p class="text-sm font-weight-normal mb-0">' . $data['cus_em_id'] . '</p></td>';
            echo '<td><p class="text-sm mb-0">' . $data['type'] . '</p></td>';
            echo '<td><p class="text-sm mb-0">' . $data['cus_em_name'] . '</p></td>';
            echo '<td><p class="text-sm mb-0">' . $data['dob'] . '</p></td>';
            echo '<td><p class="text-sm mb-0">' . $data['email'] . '</p></td>';
            echo '<td><p class="text-sm mb-0">' . $data['tel1'] . '</p></td>';
            echo '<td><p class="text-sm mb-0">' . $data['tel2'] . '</p></td>';
            echo '<td class="align-middle text-center text-sm"><p class="mb-0 text-sm">' . $data['address'] . '</p></td>';
            echo '<td class="align-middle text-center text-sm"><p class="mb-0 text-sm">' . $data['note'] . '</p></td>';
            echo '<td class="align-middle text-center">';
            echo '<a href="../account/editClientEmployee.php?id=' . $data['cus_em_id'] . '"><button class="btn">Edit</button></a>';
            echo '</td>';
            echo '</tr>';
        }
    } else {
