...
// Check if a customer_id is provided in the query string
if(isset($_GET['customer_id'])) {
    // Assuming you have already established a database connection

    // Sanitize and get the customer_id from the query string
    $customer_id = mysqli_real_escape_string($conn, $_GET['customer_id']);

    // Construct the SQL query to retrieve the customer data
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Sanitize and get the updated data from the form
    ...
    if (mysqli_query($conn, $sql)) {
        // Update successful, you can redirect the user or display a success message
        echo '<script>alert("Success");</script>';

         logEvent($_SESSION['user'], 'Update Customer Data', 'Customer ID: ' . $customer_id, 3);
        header("Location: ../../dashboards/customer_data.php");
        exit();
    ...
    $ce_name = $dob = $email = $tel1 = $tel2 = $type = $company = $address = $note = "";
    
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        // Validate and sanitize user inputs
        $cus_em_id = isset($_POST['cus_em_id']) ? $_POST['cus_em_id'] : null;
        $ce_name = isset($_POST['ce_name']) ? mysqli_real_escape_string($conn, $_POST['ce_name']) : "";
        $dob = isset($_POST['dob']) ? mysqli_real_escape_string($conn, $_POST['dob']) : "";
        $email = isset($_POST['email']) ? mysqli_real_escape_string($conn, $_POST['email']) : "";
        $tel1 = isset($_POST['tel1']) ? mysqli_real_escape_string($conn, $_POST['tel1']) : "";
        $tel2 = isset($_POST['tel2']) ? mysqli_real_escape_string($conn, $_POST['tel2']) : "";
        ...
        // Check if cus_em_id is provided
        if ($cus_em_id === null) {
            echo "Error: Missing cus_em_id.";
            exit;
        }
    
        // Update customer_employee table
        $update_ce_query = "UPDATE customer_employee SET cus_em_name=?, dob=?, email=?, tel1=?, tel2=?, type=?, note=? WHERE cus_em_id=?";
        $update_ce_stmt = mysqli_prepare($conn, $update_ce_query);
        mysqli_stmt_bind_param($update_ce_stmt, 'sssssssi', $ce_name, $dob, $email, $tel1, $tel2, $type, $note, $cus_em_id);
    
        // Update customer table
        $update_c_query = "UPDATE customer SET company_name=?, address=? WHERE customer_id=?";
        $update_c_stmt = mysqli_prepare($conn, $update_c_query);
        mysqli_stmt_bind_param($update_c_stmt, 'ssi', $company, $address, $cus_em_id);
    
        // Perform the updates within a transaction for data consistency
        mysqli_begin_transaction($conn);
    
        $update_ce_success = mysqli_stmt_execute($update_ce_stmt);
        $update_c_success = mysqli_stmt_execute($update_c_stmt);
    
        if ($update_ce_success && $update_c_success) {
            // If both updates were successful, commit the transaction
            mysqli_commit($conn);
            echo "Update successful!";
        } else {
            // If any update failed, rollback the transaction
            mysqli_rollback($conn);
            echo "Update failed!";
        }
    
        // Close the prepared statements
        mysqli_stmt_close($update_ce_stmt);
            ...
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm text-dark active" aria-current="page"> <a href="../../dashboards/customer_data.php" >Clients</a> </li>
                    </ol>
                                    ...
                                    <div class="card-body">
                                        <form class="multisteps-form__form" method="POST">
                                            ...
                                            <div class="multisteps-form__panel border-radius-xl bg-white js-active"
                                                ...
                                                <h5 class="font-weight-bolder mb-0">About Customer</h5>
                                                ...
                                                <div class="multisteps-form__content">
                                                        ...
                                                        <div class="col-12 col-sm-6 mt-3 mt-sm-0">
                                                            <div class="input-group input-group-dynamic">
                                                                ...
                                                                <input class="multisteps-form__input form-control"
                                                                    type="text" name="id"
                                                                    value="<?php echo isset($row['customer_id']) ? $row['customer_id'] : ''; ?>" hidden />
                                                                <input class="multisteps-form__input form-control"
                                                            ...
                                                            <div class="input-group input-group-dynamic">
                                                                ...
                                                                <select class="form-control" name="type"
                                                                    ...
                                                                    <option value="partner"
                                                                        <?php if (isset($row['type']) && $row['type'] === 'partner') echo 'selected'; ?>>
                                                                        Partner</option>
                                                                    <option value="end_customer"
                                                                        <?php if (isset($row['type']) && $row['type'] === 'end_customer') echo 'selected'; ?>>
                                                                        End Customer</option>
                                                                </select>
                                                        ...
                                                        <div class="col-12 col-sm-6 mt-3 mt-sm-0">
                                                            <div class="input-group input-group-dynamic">
                                                                ...
                                                                <input class="multisteps-form__input form-control"
                                                                    ...
                                                                    value="<?php echo isset($row['payment_terms']) ? $row['payment_terms'] : ''; ?>"></input>
                                                            </div>
                                                        </div>
                                                        <div class="col-12 col-sm-6 mt-3 mt-sm-0">
                                                            <div class="input-group input-group-dynamic">
                                                                <label class="form-label">VAT Num</label>
                                                                <input class="multisteps-form__input form-control"
                                                                    type="text" name="vat"
                                                                    value="<?php echo isset($row['vat']) ? $row['vat'] : ''; ?>"></input>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="row">
                                                    <div class="col-12 col-sm-12 mt-5 mt-sm-3">
                                                            <div class="input-group input-group-dynamic">
                                                                ...
                                                                <input class="multisteps-form__input form-control"
                                                                    type="text" name="note"
                                                                    value="<?php echo isset($row['note']) ? $row['note'] : ''; ?>"></input>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                    ...
                    <div class="row align-items-center justify-content-lg-between">
                        ...
                        <div class="col-lg-6">
                            <ul class="nav nav-footer justify-content-center justify-content-lg-end">
                                ...
                                <li class="nav-item">
                                    <a href="https://www.activelk.com/presentation" class="nav-link text-muted"
                                        target="_blank">About Us</a>
                                </li>
                                <li class="nav-item">
                                    <a href="https://www.activelk.com/blog" class="nav-link text-muted"
