...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Insert data into the customer_employee table
    ...
    if (mysqli_query($conn, $insert_ce_query)) {
        // Get the last inserted customer_employee ID
        $ce_id = mysqli_insert_id($conn);

        // Retrieve the customer_id for the selected company_name from the customer table
        ...
        if ($customer_result && mysqli_num_rows($customer_result) > 0) {
            ...
            // Insert the data into the cus_emp_junc table
            ...
            if (mysqli_query($conn, $insert_junc_query)) {
                $message = "New customer_employee inserted successfully.";

                $logSql = "INSERT INTO log (user, task, date_time, other, serverity) VALUES (?, ?, NOW(), ?, ?)";
                $logStmt = $conn->prepare($logSql);
            
                $user = $_SESSION['user'];
                    $task = 'Add Customer Employee';
                    $other = ' CE Name :  ' . $ce_name ;
                ...
                $message = "Data not inserted into the junction table: " . mysqli_error($conn);
            }
        } else {
            $message = "Error fetching customer_id: " . mysqli_error($conn);
        }
    } else {
        $message = "Error inserting data into the customer_employee table: " . mysqli_error($conn);
    }

    // Close the database connection
    mysqli_close($conn);

    // Redirect to a confirmation page with the message
    header("Location:../../../pages/pages/account/newClientEmployee.php" );
            ...
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm text-dark active" aria-current="page"><a href="../../pages/account/customerEmployeeDetails.php">Client Employee</a> </li>
                    </ol>
                    <h6 class="font-weight-bolder mb-0">New CE</h6>
                </nav>
