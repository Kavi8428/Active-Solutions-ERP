...
// Fetch employee names based on employee IDs
$data = [];
if (!empty($empIDs)) {
    $empIDList = implode(",", $empIDs);
    $cusEmpName = $conn->query("SELECT cus_em_name FROM customer_employee WHERE cus_em_id IN ($empIDList)");

    if ($cusEmpName->num_rows > 0) {
        while ($row = $cusEmpName->fetch_assoc()) {
            $data[] = $row['cus_em_name'];
        }
    } else {
...
$responseData = array(
    'cusEmpName' => $data // Keep $data as an array of names
);
